...
public class EdicionU extends javax.swing.JFrame {
    long id_u;
    ControladoraL control;
    Usuario usu;
    
    public EdicionU(ControladoraL control,long id_u) {
        initComponents();
        this.id_u = id_u;
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        usu = control.TraerUsuario(id_u);
        txtNombreU.setText(usu.getNombreU());
        txtContrasena.setText(usu.getContrasenaU());
        
        List<Rol> listaRoles = control.TraerRoles();
        if(listaRoles != null){    
            for(Rol i : listaRoles){
                cmbRoles.addItem(i.getNombreR());
            }
        }
        
        String rol = usu.getUnRol().getNombreR();
        
        long cantidadI = cmbRoles.getItemCount();
        for(int i = 0; i < cantidadI ; i++){
            if(String.valueOf(cmbRoles.getItemAt(i)).equals(rol)){
